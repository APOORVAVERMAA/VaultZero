...
exports.getProfile = async (req, res) => {
  try {
    ...
    const { rows: users } = await db.query(
      'SELECT id, email, last_login, created_at FROM users WHERE id = $1',
      [userId]
    );
...
exports.getActivity = async (req, res) => {
  try {
    ...
    const { rows: events } = await db.query(
      'SELECT id, vault_id, event_type, ip_address, created_at FROM vault_events WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50',
      [userId]
    );
    res.json(events);
  } catch (error) {
...
exports.deleteAccount = async (req, res) => {
  ...
  try {
    ...
    await conn.query('DELETE FROM security_questions WHERE user_id = $1', [userId]);
    // 2. Delete release tokens (linked via vault_id)
    await conn.query('DELETE FROM release_tokens WHERE vault_id IN (SELECT id FROM vaults WHERE user_id = $1)', [userId]);
    // 3. Delete vault events
    await conn.query('DELETE FROM vault_events WHERE user_id = $1', [userId]);
    // 4. Delete vaults
    await conn.query('DELETE FROM vaults WHERE user_id = $1', [userId]);
    ...
    if (conn && transactionStarted) {
      try { await conn.query('ROLLBACK'); } catch (rbErr) {
