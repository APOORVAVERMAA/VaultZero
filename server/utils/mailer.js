...
const { wrapEmail, infoTable, ctaButton, heading, subtext, badge, noteBox, warningBox, esc } = require("./emailTemplate");
...
const sendReleaseEmail = async (to, vault, ownerEmail, releaseLink) => {
  const body = `
    ...
    <div style="margin-top:20px;">
      <p style="color:#6b7280;font-size:11px;margin:4px 0;line-height:1.6;">&#8226; This link can be used only once</p>
      <p style="color:#6b7280;font-size:11px;margin:4px 0;line-height:1.6;">&#8226; It expires in 48 hours</p>
      <p style="color:#6b7280;font-size:11px;margin:4px 0;line-height:1.6;">&#8226; All access attempts are logged and monitored</p>
    </div>
    ${warningBox('VaultZero does NOT store or know the original passphrase. Without the correct passphrase, decryption is cryptographically impossible.')}
    ${noteBox('Encryption: AES-256-GCM &#8226; PBKDF2 Key Derivation &#8226; HMAC Integrity Verification')}
  `;

  await transporter.sendMail({
    from: `"VaultZero Security" <${process.env.EMAIL_USER}>`,
    to,
    subject: "VaultZero — Vault Released: Secure Access Link",
    html: wrapEmail(body),
  });
};

module.exports = { sendReleaseEmail, sendVerificationEmail };